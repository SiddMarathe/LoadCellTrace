...
def mainFunc():
    try:
        ...
        while client.connect():
            ...
            # if startSignal == 1:
            result = client.read_holding_registers(address=128+4096,count=2,unit=1)
            ...
            if currentTime >= monitorTime:
                ...
                fltBuffer = []
                for i in range(0,len(resBuffer),2):
                    decoder = BinaryPayloadDecoder.fromRegisters(resBuffer[i], Endian.Big, wordorder=Endian.Little)
                    fltBuffer.append(decoder.decode_32bit_float())
                for o in range(len(fltBuffer)):
                    ex_data=[RTC.RTC().date(),o*18,fltBuffer[o]]
                    # print(ex_data)
                    ex_headers=['Date','ms','Weight']
        ...
        plt.plot([i*18 for i in range(len(fltBuffer))],fltBuffer)
